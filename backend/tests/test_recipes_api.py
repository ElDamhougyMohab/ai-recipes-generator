...
class TestRecipeAPI:
    ...
    def test_create_recipe_invalid_difficulty(self, client: TestClient, sample_recipe_data):
    ...
    def test_create_recipe_invalid_ingredient_format(self, client: TestClient, sample_recipe_data):
    ...
    def test_create_recipe_unicode_ingredients(self, client: TestClient, sample_recipe_data):
        """Test recipe creation with unicode characters"""
        sample_recipe_data["ingredients"] = [
            {"name": "Jalapeño peppers", "amount": "2", "unit": "pieces", "notes": "fresh"},
            {"name": "Café au lait", "amount": "1", "unit": "cup", "notes": "for breakfast"},
    ...
    def test_rate_recipe_invalid_rating(self, client: TestClient, created_recipe):
        ...
        response = client.put(f"/api/recipes/{recipe_id}/rating", params={"rating": 6.0})
        assert response.status_code == 422
        
        # Test rating too low
        response = client.put(f"/api/recipes/{recipe_id}/rating", params={"rating": -1.0})
        assert response.status_code == 422
    ...
    def test_create_recipe_large_payload(self, client: TestClient):
        ...
        large_recipe = {
            ...
            "ingredients": [
                {"name": f"ingredient_{i}", "amount": str(i), "unit": "cup", "notes": f"notes_{i}"}
