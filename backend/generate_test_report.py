...
class TestReportGenerator:
    ...
    def create_charts(self, test_data: Dict[str, Any]) -> List[str]:
        ...
        if test_data["tests"]:
            ...
            for test in test_data["tests"]:
                file_name = test["nodeid"].split("::")[0].replace("tests/", "").replace(".py", "")
            ...
            if test_files:
                ...
                ax.bar([i - width for i in x], passed_counts, width, label='Passed', color='#28a745')
                ax.bar(x, failed_counts, width, label='Failed', color='#dc3545')
                ax.bar([i + width for i in x], skipped_counts, width, label='Skipped', color='#ffc107')
                ...
                bar_chart_path = self.output_dir / f"test_results_bar_{self.timestamp}.png"
                plt.tight_layout()
    ...
    def generate_pdf_report(self, test_data: Dict[str, Any], chart_files: List[str]) -> str:
        ...
        summary_data = [
            ...
            ["Success Rate", f"{(summary['passed'] / max(summary['total'], 1)) * 100:.1f}%"],
            ...
            for file_name, tests in test_by_file.items():
                ...
                for test in tests:
                    ...
                    if description != "Test description not available":
                        story.append(Paragraph(f"<b>Purpose:</b> {description}", self.normal_style))
                    
                    # Failure details for failed tests
                    if status == "FAILED":
                        failure_details = self._extract_failure_details(test)
                        
                        story.append(Paragraph(f"<b>❌ Failure Stage:</b> {failure_details['stage'].title()}", self.normal_style))
                        story.append(Paragraph(f"<b>Error Type:</b> {failure_details['error_type']}", self.normal_style))
                        story.append(Paragraph(f"<b>Error Message:</b> {failure_details['error_message']}", self.normal_style))
                        ...
                        if failure_details['stack_trace']:
                            ...
                            relevant_lines = []
                            for line in trace_lines[-10:]:  # Last 10 lines are usually most relevant
                                if line.strip() and not line.startswith('='):
                                    relevant_lines.append(line.strip())
                            
                            if relevant_lines:
                                story.append(Paragraph("<b>Stack Trace (excerpt):</b>", self.normal_style))
                                trace_text = '<br/>'.join(relevant_lines[:5])  # Limit to 5 lines
                                story.append(Paragraph(f"<font name='Courier' size='8'>{trace_text}</font>", self.normal_style))
        ...
        env_data = [
            ["Python Version", test_data.get("environment", {}).get("Python", "Unknown")],
                    ...
                    if isinstance(longrepr, str):
                        ...
                        for line in lines:
                            if "Error:" in line or "Exception:" in line:
                                ...
                                failure_info["error_message"] = ':'.join(line.split(':')[1:]).strip()
                                break
                        failure_info["stack_trace"] = longrepr
                    elif isinstance(longrepr, dict) and "reprcrash" in longrepr:
                        crash = longrepr["reprcrash"]
                        failure_info["error_message"] = crash.get("message", "Unknown error")
                        failure_info["error_type"] = crash.get("path", "").split('.')[-1]
                break
