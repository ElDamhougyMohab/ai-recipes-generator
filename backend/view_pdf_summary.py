...
def show_latest_report_summary():
    ...
    try:
        ...
        print(f"   🎯 Success Rate: {(summary.get('passed', 0) / max(summary.get('total', 1), 1)) * 100:.1f}%")
        print()
        ...
        passed_tests = [test for test in data.get("tests", []) if test.get("outcome") == "passed"]
        if passed_tests:
        ...
        pdf_file = latest_json.name.replace("test_data_", "test_report_").replace(".json", ".pdf")
