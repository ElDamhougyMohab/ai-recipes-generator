...
async def validate_dietary_restrictions(request: schemas.RecipeGenerateRequest):
    """Validate ingredients against dietary restrictions and return conflicts"""
    try:
        gemini_service = GeminiService()
        allowed_ingredients, protein_suggestions = (
            gemini_service._filter_ingredients_by_diet(
                request.ingredients, request.dietary_preferences
            )
        )

        forbidden_ingredients = [
            ing for ing in request.ingredients if ing not in allowed_ingredients
        ]
        ...
        return {
            ...
            "message": (
                "Some ingredients conflict with your dietary preferences"
                if forbidden_ingredients
                else "All ingredients are compatible"
            ),
        }
...
async def generate_recipes(
    request: schemas.RecipeGenerateRequest, db: Session = Depends(get_db)
):
    try:
        gemini_service = GeminiService()
        ...
        return {
            ...
            "dietary_filtering": {
                ...
                "protein_suggestions": protein_suggestions,
                "message": (
                    f"Filtered out {len(forbidden_ingredients)} non-compliant ingredients"
                    if forbidden_ingredients
                    else "All ingredients are dietary compliant"
                ),
            },
            "generation_info": {
...
def read_recipes(
    ...
    page_size: int = Query(
        10, ge=1, le=100, description="Number of recipes per page (max 100)"
    ),
    ...
    try:
        ...
        paginated_result = crud.get_recipes_paginated(
            db=db, page=page, per_page=page_size
        )
...
def search_recipes(
    ...
    max_prep_time: Optional[int] = Query(
        None, ge=0, le=600, description="Maximum prep time in minutes"
    ),
    max_cook_time: Optional[int] = Query(
        None, ge=0, le=1440, description="Maximum cook time in minutes"
    ),
    page: int = Query(1, ge=1, le=1000, description="Page number (1-based)"),
    page_size: int = Query(
        10, ge=1, le=100, description="Number of recipes per page (max 100)"
    ...
    try:
        ...
        paginated_result = crud.get_recipes_paginated(
            db=db, page=page, per_page=page_size, search=search_params.q
        )
...
def rate_recipe(
    ...
    try:
        updated_recipe = crud.update_recipe_rating(
            db, recipe_id=recipe_id, rating=rating_request.rating
        )
...
def get_stats(db: Session = Depends(get_db)):
    ...
    return {"total_recipes": total_recipes, "average_rating": round(avg_rating, 2)}
...
def read_meal_plans(
    page: int = Query(1, ge=1, le=1000, description="Page number (1-based)"),
    page_size: int = Query(
        10, ge=1, le=100, description="Number of meal plans per page (max 100)"
    ),
    db: Session = Depends(get_db),
):
