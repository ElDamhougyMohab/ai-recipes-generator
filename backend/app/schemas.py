...
class IngredientSchema(BaseModel):
    ...
    amount: Optional[Union[int, float, str]] = Field(
        None, description="Amount of ingredient"
    )
...
class RecipeBase(BaseModel):
    ...
    ingredients: List[IngredientSchema] = Field(
        ..., min_items=1, max_items=50, description="List of ingredients"
    )
    prep_time: Optional[int] = Field(
        None, ge=0, le=600, description="Preparation time in minutes"
    )
    cook_time: Optional[int] = Field(
        None, ge=0, le=1440, description="Cooking time in minutes"
    )
...
class RecipeGenerateRequest(BaseModel):
    ingredients: List[str] = Field(
        ..., min_items=1, max_items=30, description="List of ingredients"
    )
    dietary_preferences: Optional[List[str]] = Field(
        default=[], max_items=10, description="Dietary preferences"
    )
    cuisine_type: Optional[str] = Field(
        None, max_length=50, description="Type of cuisine"
    )
    ...
    def validate_dietary_preferences(cls, v):
        ...
        for pref in v:
            ...
            if clean_pref not in valid_diets:
                raise ValueError(
                    f'Invalid dietary preference: {pref}. Valid options: {", ".join(valid_diets)}'
                )
    ...
    def validate_cuisine_type(cls, v):
        ...
        if clean_cuisine not in valid_cuisines:
            raise ValueError(
                f'Invalid cuisine type: {v}. Valid options: {", ".join(valid_cuisines)}'
            )
    ...
    def validate_meal_type(cls, v):
        ...
        if clean_meal not in valid_meal_types:
            raise ValueError(
                f'Invalid meal type: {v}. Valid options: {", ".join(valid_meal_types)}'
            )
    ...
    def validate_recipes(cls, v):
        ...
        for day, recipe_ids in v.items():
            ...
            if len(recipe_ids) > 10:
                raise ValueError(
                    f"Too many recipes for {day}. Maximum 10 recipes per day"
                )

            for recipe_id in recipe_ids:
                if not isinstance(recipe_id, int) or recipe_id <= 0:
                    raise ValueError(
                        f"Invalid recipe ID: {recipe_id}. Must be a positive integer"
                    )
...
class PaginationParams(BaseModel):
    skip: int = Field(
        default=0, ge=0, le=10000, description="Number of records to skip"
    )
    limit: int = Field(
