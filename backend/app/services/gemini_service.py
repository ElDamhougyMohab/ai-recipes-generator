...
class GeminiService:
    def __init__(self):
        ...
        self.dietary_restrictions = {
            'vegetarian': {
                'forbidden_ingredients': ['chicken', 'beef', 'pork', 'lamb', 'turkey', 'duck', 'fish', 'salmon', 'tuna', 'shrimp', 'crab', 'lobster', 'meat', 'bacon', 'ham', 'sausage', 'pepperoni'],
                'allowed_proteins': ['tofu', 'tempeh', 'beans', 'lentils', 'chickpeas', 'quinoa', 'nuts', 'seeds', 'eggs', 'dairy']
            },
            'vegan': {
                'forbidden_ingredients': ['chicken', 'beef', 'pork', 'lamb', 'turkey', 'duck', 'fish', 'salmon', 'tuna', 'shrimp', 'crab', 'lobster', 'meat', 'bacon', 'ham', 'sausage', 'pepperoni', 'eggs', 'milk', 'cheese', 'butter', 'yogurt', 'cream'],
                'allowed_proteins': ['tofu', 'tempeh', 'beans', 'lentils', 'chickpeas', 'quinoa', 'nuts', 'seeds', 'nutritional yeast']
            },
            'gluten-free': {
                'forbidden_ingredients': ['wheat', 'barley', 'rye', 'flour', 'bread', 'pasta', 'noodles', 'soy sauce'],
                'allowed_alternatives': ['rice', 'quinoa', 'gluten-free flour', 'corn', 'potatoes']
            },
            'dairy-free': {
                'forbidden_ingredients': ['milk', 'cheese', 'butter', 'yogurt', 'cream', 'ice cream'],
                'allowed_alternatives': ['almond milk', 'coconut milk', 'vegan cheese', 'coconut oil']
            }
        }

    def _filter_ingredients_by_diet(self, ingredients: List[str], dietary_preferences: Optional[List[str]] = None) -> tuple[List[str], List[str]]:
        ...
        for ingredient in ingredients:
            ...
            for diet in dietary_preferences:
                ...
                if diet_lower in self.dietary_restrictions:
                    forbidden_list = self.dietary_restrictions[diet_lower].get('forbidden_ingredients', [])
                    if any(forbidden_item in ingredient_lower for forbidden_item in forbidden_list):
                        is_forbidden = True
                        forbidden_ingredients.append(ingredient)
                        
                        # Add suggestions for replacements
                        if diet_lower in ['vegetarian', 'vegan'] and any(meat in ingredient_lower for meat in ['chicken', 'beef', 'pork', 'meat']):
                            suggestions.extend(self.dietary_restrictions[diet_lower]['allowed_proteins'])
                        break
    ...
    async def generate_recipes(
        ...
        allowed_ingredients, protein_suggestions = self._filter_ingredients_by_diet(ingredients, dietary_preferences)
        
        prompt = self._build_prompt(allowed_ingredients, dietary_preferences, cuisine_type, meal_type, protein_suggestions)
        ...
        try:
            ...
            validated_recipes = self._validate_recipes_against_diet(recipes_data, dietary_preferences)
...
"""
    ...
    def _validate_recipes_against_diet(self, recipes: List[Dict[str, Any]], dietary_preferences: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        ...
        for recipe in recipes:
            ...
            for ingredient in recipe.get('ingredients', []):
                ...
                for diet in dietary_preferences:
                    ...
                    if diet_lower in self.dietary_restrictions:
                        forbidden_list = self.dietary_restrictions[diet_lower].get('forbidden_ingredients', [])
                        if any(forbidden_item in ingredient_name for forbidden_item in forbidden_list):
    ...
    def _parse_text_response(self, text: str) -> List[Dict[str, Any]]:
        ...
        for section in recipe_sections:
            ...
            recipe = {
                ...
                "ingredients": [{"name": "Various ingredients", "amount": "As needed", "unit": ""}],
                "prep_time": 20,
                "cook_time": 30,
                "servings": 4,
                "difficulty": "Medium"
            ...
            title_match = re.search(r'(?:Recipe \d+:|^\d+\.|^\*\*)(.*?)(?:\n|\*\*)', section)
            if title_match:
    ...
    def _get_fallback_recipes(self, ingredients: List[str], dietary_preferences: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        # Filter ingredients based on dietary restrictions
        allowed_ingredients, _ = self._filter_ingredients_by_diet(ingredients, dietary_preferences)
        
        if not allowed_ingredients:
            # If no ingredients are allowed, create a basic recipe with alternatives
            if dietary_preferences and 'vegetarian' in [d.lower() for d in dietary_preferences]:
                allowed_ingredients = ['tofu', 'vegetables', 'rice']
            elif dietary_preferences and 'vegan' in [d.lower() for d in dietary_preferences]:
        ...
        if dietary_preferences and 'vegetarian' in [d.lower() for d in dietary_preferences]:
            fallback_recipes.append({
                ...
                "ingredients": [
                    {"name": ing, "amount": "1 cup", "unit": "cup"} for ing in allowed_ingredients
            ...
            fallback_recipes.append({
                ...
                "ingredients": [
                    {"name": ing, "amount": "1 cup", "unit": "cup"} for ing in allowed_ingredients
                ] + [{"name": "quinoa", "amount": "1 cup", "unit": "cup"}, {"name": "olive oil", "amount": "2 tbsp", "unit": "tbsp"}],
                "prep_time": 15,
                "cook_time": 20,
                "servings": 2,
                "difficulty": "Easy"
