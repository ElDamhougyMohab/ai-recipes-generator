...
const PDFSubtitle = styled.p`
  color: #666;
  margin: 0;
  font-size: 14px;
`;

const PDFWeekGrid = styled.div`
  display: grid;
  grid-template-columns: repeat(7, 1fr);
  gap: 15px;
  margin-bottom: 30px;
...
const PDFMealType = styled.div`
  font-weight: bold;
  color: #667eea;
  margin-bottom: 4px;
`;

const PDFRecipeTitle = styled.div`
  color: #333;
  line-height: 1.3;
`;

const PDFIngredientsList = styled.div`
  margin-top: 30px;
  page-break-inside: avoid;
`;

const PDFSectionTitle = styled.h2`
  color: #333;
  margin: 0 0 20px 0;
  font-size: 20px;
  padding-bottom: 10px;
...
const PDFIngredientItem = styled.div`
  padding: 8px 0;
  border-bottom: 1px solid #f0f0f0;
  font-size: 12px;
`;

const PDFRecipeDetails = styled.div`
  margin-top: 30px;
  page-break-inside: avoid;
`;

const PDFRecipeCard = styled.div`
  margin-bottom: 25px;
  padding: 20px;
  border: 1px solid #dee2e6;
  border-radius: 8px;
...
const PDFRecipeCardTitle = styled.h3`
  color: #333;
  margin: 0 0 15px 0;
  font-size: 16px;
`;

const PDFRecipeInfo = styled.div`
  display: flex;
  justify-content: space-between;
  margin-bottom: 15px;
  font-size: 11px;
...
const PDFInstructions = styled.div`
  font-size: 12px;
  line-height: 1.4;
  color: #333;
`;

const PDFRecipeIngredients = styled.div`
  margin-bottom: 15px;
  font-size: 11px;
`;

const PDFRecipeIngredientItem = styled.div`
  padding: 2px 0;
  color: #333;
`;
  ...
  const saveMealPlan = async () => {
    ...
    try {
      ...
      for (const tempRecipe of tempRecipes) {
        try {
          // Create recipe data without temporary fields
          const recipeData = {
    ...
    try {
      ...
      for (const [day, recipeIds] of Object.entries(plan.recipes)) {
        if (recipeIds && recipeIds.length > 0) {
          // For simplicity, we'll assign recipes to meals in order
