...
import { InlineLoader, LoadingOverlay } from './LoadingIndicators';
...
function MealPlanner({ addNotification }) {
  ...
  const [allRecipes, setAllRecipes] = useState([]); // Load ALL recipes for meal planning
  ...
  const loadAllRecipes = async () => {
    try {
      ...
      // Load all recipes by fetching all pages
      while (hasMore) {
        const response = await recipeAPI.getRecipes(currentPage, 100); // Use max page size
        allRecipesData.push(...response.items);
      ...
      addNotification?.('Failed to load recipes for meal planning', 'error');
      setAllRecipes([]);
    } finally {
  ...
  const saveMealPlan = async () => {
    if (!planName.trim()) {
      addNotification('Please enter a name for your meal plan', 'warning');
      return;
    }
    ...
    try {
      ...
      const planData = {
        ...
        recipes: Object.keys(updatedMealPlan).reduce((acc, day) => {
          const dayRecipes = Object.values(updatedMealPlan[day])
            ...
            .map(recipe => {
              // Debug: Log recipe structure to identify issues
              if (!recipe || !recipe.id) {
                console.error('Invalid recipe found in meal plan:', recipe);
                addNotification('Invalid recipe found in meal plan. Please check your selections.', 'error');
              ...
              if (isNaN(recipeId) || recipeId <= 0) {
                console.error('Invalid recipe ID:', recipe.id, 'for recipe:', recipe);
                addNotification(`Invalid recipe ID: ${recipe.id}. Please re-select the recipe.`, 'error');
      ...
      addNotification('✅ Meal plan saved successfully!', 'success');
    } catch (error) {
      console.error('Error saving meal plan:', error);
      addNotification(`❌ Failed to save meal plan: ${error.userMessage || error.message}`, 'error');
    } finally {
      setSavingPlan(false);
      setIsLoading(false);
    }
  ...
  const loadMealPlan = async (plan) => {
    try {
      ...
      addNotification(`❌ Failed to load meal plan: ${error.userMessage || error.message}`, 'error');
    }
  };
  ...
  const deleteMealPlan = async (planId) => {
    ...
    try {
      ...
      await loadSavedPlans();
      addNotification('✅ Meal plan deleted successfully!', 'success');
    } catch (error) {
      console.error('Error deleting meal plan:', error);
      addNotification(`❌ Failed to delete meal plan: ${error.userMessage || error.message}`, 'error');
    }
  };
  ...
  const exportToPDF = async (plan = null) => {
    try {
      ...
      if (plan) {
        ...
        for (const [, recipeIds] of Object.entries(plan.recipes)) {
      ...
      for (let i = 0; i < days.length; i++) {
        ...
        const dayMeals = plan ? 
          (plan.recipes[day] || []).map(id => allRecipes.find(r => r.id === id)).filter(Boolean) :
      ...
      addNotification(`❌ Failed to generate PDF: ${error.userMessage || error.message}`, 'error');
    } finally {
  ...
  if (allRecipes.length === 0) {
    return (
      <Container>
        ...
        <EmptyState>
          {loadingRecipes ? (
            <>📥 Loading recipes for meal planning...</>
          ) : (
            <>
    ...
    <Container>
                ...
                {showSelector?.day === day && showSelector?.mealTime === mealTime && (
                  <RecipeSelector>
                    {loadingRecipes ? (
                      ...
                      <div style={{ padding: '20px', textAlign: 'center' }}>
                        <p>No recipes available. Generate some recipes first!</p>
                      </div>
                    ) : (
      ...
      <ButtonGroup>
        ...
        <SecondaryButton onClick={() => exportToPDF()} disabled={generatingPDF}>
          {generatingPDF ? (
            <>
              <InlineLoader type="dots" /> Generating PDF...
            </>
          ) : (
            '📄 Export PDF'
          )}
        </SecondaryButton>
        <SaveButton onClick={saveMealPlan} disabled={isLoading || savingPlan}>
          {savingPlan ? (
            <>
              <InlineLoader type="wave" color="white" /> Saving...
            </>
          ) : (
            '💾 Save Meal Plan'
          )}
      ...
      <SavedPlansSection>
        ...
        {loadingPlans ? (
          <div style={{ textAlign: 'center', padding: '20px' }}>
            <InlineLoader type="wave" text="Loading saved plans..." />
          </div>
        ) : savedPlans.length === 0 ? (
          <EmptyState>
            ...
            <SavedPlanCard key={plan.id}>
              <PlanHeader>
                ...
                <div>
                  ...
                  <ExportButton onClick={() => exportToPDF(plan)} disabled={generatingPDF}>
                    {generatingPDF ? <InlineLoader type="dots" /> : '📄 Export PDF'}
                  </ExportButton>
